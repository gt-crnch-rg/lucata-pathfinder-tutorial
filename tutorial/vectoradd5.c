...
int main(int argc, char **argv)
{
  ...
  long **x = mw_malloc2d(size, blk * sizeof(long));
  long **y = mw_malloc2d(size, blk * sizeof(long));
  long **z = mw_malloc2d(size, blk * sizeof(long));
  ...
  for (long i = 0; i < size; i++) {
    cilk_spawn_at (z[i]) vectoradd(blk, x[i], y[i], z[i]);
  } cilk_sync;
